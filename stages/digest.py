...
{rundown_block}{_section("Awaiting your approval (panel: localhost:8787)", ra.get("queued", []) + org.get("queued", []))}{_section("Replies detected (action: respond!)", sync.get("replies"))}{_section("Sends detected (you sent these manually)", sync.get("sent_detected"))}{_section("OOO auto-replies (not counted as replies)", sync.get("ooo"))}{_section("Bounces detected", sync.get("bounces"))}{_section("Bounces fixed (corrected drafts in Gmail)", br.get("fixed"))}{_section("Bounces needing manual send (verified address found)", br.get("manual"))}{_section("Bounces dead (no confident fix)", br.get("dead"))}{_section("Reply nudges drafted (they promised, went silent — review + send)", nu.get("drafted"))}{_section("We owe them contact later (calendar it)", nu.get("we_owe"))}{_section("Nudges skipped", nu.get("skipped"))}{_section("Follow-up drafts created (review + send in Gmail Drafts)", fu.get("drafted"))}{_section("Follow-ups skipped", fu.get("skipped"))}{_section("New prospect briefs (queue/prospects/)", pr.get("briefs"))}{_section("Prospects rejected by confidence bar", pr.get("rejected"))}{_section("Backfill ambiguous (manual thread check needed)", sync.get("backfill_ambiguous"))}{_section("Errors", all_errors)}
## Pipeline
