...
# Hard-stop at TOKEN_HARD_PCT so other Claude use keeps the remaining half.
...
SCOUT_METER_MAX_PCT = 0.32
# Skip organize LLM work when meter is this high (still ok to no-op).
ORGANIZE_METER_MAX_PCT = 0.42
# Skip bounce research when meter is this high.
BOUNCE_METER_MAX_PCT = 0.42
# Skip reply drafts when meter is this high (inbox sync still ran).
REPLY_METER_MAX_PCT = 0.42
# If this many unorganized briefs are waiting, organize them and skip scout.
SCOUT_SKIP_IF_BRIEFS_WAITING = 2
# Cheaper scout tool loops.
SCOUT_BRIEF_MAX_TURNS = 5
SCOUT_DISCOVERY_MAX_TURNS = 6
...
RETRY_METER_MAX_PCT = 0.40
# Cap Exa amplification: research at most this many discovery hits per track.
SCOUT_MAX_CANDIDATES_PER_DISCOVERY = 2
# Cap how many outreach drafts organize writes per run (rest wait).
ORGANIZE_MAX_PER_RUN = 3
# Bounce research turns.
BOUNCE_MAX_TURNS = 4
# Bound reply-thread context stuffed into the LLM.
REPLY_THREAD_MAX_MSGS = 5
REPLY_THREAD_MSG_CHARS = 600
